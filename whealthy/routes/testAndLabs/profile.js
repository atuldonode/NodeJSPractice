...
router.post("/add/master", auth, (req, res) => {
...
router.get("/all/master", (req, res) => {
  log.debug("/api/getall/profilemaster");
  crudController
    .getAll(ProfileMaster)
    .then(async(testData) => {
      const data =   await Icon.find( { profileMasterId: { $exists: true }, status: { $ne: "deleted" } } );
...
router.get("/master/by/profileId/:id", (req, res) => {
  log.debug("/api/getProfileMaster");
  crudController
     .getBy(ProfileMaster, {
      _id: req.params.id
    })
    .then(async(testData) => {
      const data =   await crudController.getBy(Icon  , {profileMasterId : req.params.id})
      response.successResponse(res, 200, {testData , data});
    })
    .catch((error) => {
      log.error(error);
  ...
  crudController
    ...
    .then(async(testData) => {
      ...
      req.body.iconArray.forEach(element => {
        arrIcon.push({
          fileURL:element,
          profileMasterId : req.params.id
        })
      });
     await crudController.insertMultiple(Icon  , arrIcon)
     var userData = await crudController.getOne(ProfileMaster, { _id: req.params.id })
     const data =   await crudController.getBy(Icon  , {profileMasterId : req.params.id})
     response.successResponse(res, 200, {userData , data});
    })
    .catch((error) => {
      log.error(error);
      response.errorResponse(res, 500);
...
router.delete("/master/:id", auth, (req, res) => {
  ...
  crudController
    ...
    .then(async(userData) => {
      const deleteIcon = await Icon.updateMany({profileMasterId : req.params.id}, {status: "deleted"});
      crudController
        .deleteMulti(ProfileTests, {
...
router.post("/search/LabProfiles", (req, res) => {
  log.debug("/api/LabProfiles/search");
...
router.delete("/labProfile/:id", (req, res) => {
  log.debug("/api/");
  crudController
...
router.put("/labProfile/by/:id", (req, res) => {
  crudController
