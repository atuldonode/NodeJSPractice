...
module.exports = {
  register: (data) => {
    return new Promise((resolve, reject) => {
      log.debug("register");
      User.findOne({
        $or: [{$and:[{email: data.email}, {email:{$ne: null}}] }, { $and:[{mobileNumber: data.mobileNumber}, {mobileNumber: {$ne: null}}] }],
      })
        .then((resUser) => {
          console.log("===>>>" , resUser)
          if (resUser) {
            ...
            bcrypt.genSalt(saltRounds, function (err, salt) {
              bcrypt.hash(data.password, salt, function (err, hash) {
                ...
                function uID(u){
                  data["uniqueID"] = u + Date.now().toString().slice(-2) + uuidv4().slice(-6).toUpperCase();
                }
              
                if(data.designation == "Doctor"){
                  uID(unad = "DR-");
