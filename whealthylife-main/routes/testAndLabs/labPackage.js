...
const labTests = mongoose.model("labTests");
const Icon = mongoose.model("Icon");

let auth = require("../../helper/auth");
let _ = require("lodash");

router.post("/master/add", auth, (req, res) => {
  log.debug("/api/packagemaster/details");
  crudController
...
router.get("/getPackages/:name", (req, res) => {
  log.debug("/api/");
  crudController
    .getByfilterName(PackageMaster, req.params.name)
    .then((testData) => {
      response.successResponse(res, 200, testData);
    })
    .catch((error) => {
...
router.get("/getBy/packageMastertId/:id", (req, res) => {
  log.debug("/api/");
  crudController
    .getBy(PackageMaster, {
      _id: req.params.id
...
router.put("/master/by/:id", auth, (req, res) => {
  crudController
    ...
    .then(async (testData) => {
      ...
      await crudController.insertMultiple(Icon, arrIcon)
      var userData = await crudController.getOne(PackageMaster, {
        _id: req.params.id
      })
      const data = await crudController.getBy(Icon, {
        packageMasterId: req.params.id
...
router.post("/search/LabPackages", (req, res) => {
  log.debug("/api/LabPackages/search");
...
router.delete("/labPackage/:id", (req, res) => {
  log.debug("/api/");
  crudController
...
router.put("/labPackage/:id", (req, res) => {
  crudController
